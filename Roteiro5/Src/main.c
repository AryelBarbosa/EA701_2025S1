...
Perifericos_t LEDInterativo = {
		...
		//primeira sequencia: vermelho e azul
		.estado_leds[0][0]= VERMELHO
		.estado_leds[0][1]= AZUL
		.estado_leds[0][2]= VERMELHO
		.estado_leds[0][3]= AZUL
		.estado_leds[0][4]= VERMELHO
		.estado_leds[0][5]= AZUL
		.estado_leds[0][6]= VERMELHO
		.estado_leds[0][7]= AZUL
};
...
void EXTI15_10_IRQHandler(void) {
    if (EXTI->PR1 & EXTI_PR1_PR13_Msk) {
        ...
        LEDInterativo.contador = LEDInterativo.contador %5;
    ...
    if (LEDInterativo.estado_botao) {
        switch (LEDInterativo.estado_leds) {
			...
			case 0b00001000: // Azul: R = G = 0, B = 1
				GPIOD->ODR &= ~(GPIO_ODR_OD12_Msk); //Apaga o LED vermelho
				GPIOD->ODR &= ~(GPIO_ODR_OD14_Msk); //Apaga o LED verde
				GPIOD->ODR |= GPIO_ODR_OD15_Msk;    //Acende o LED azul
				LEDInterativo.cor_led = AZUL;
				break;
            case 0b00010000: // Amarelo: R = G = 1, B = 0
                GPIOD->ODR |= GPIO_ODR_OD12_Msk; // Acende vermelho
                GPIOD->ODR |= GPIO_ODR_OD14_Msk; // Acende verde
                GPIOD->ODR &= ~GPIO_ODR_OD15_Msk; // Apaga azul
                LEDInterativo.cor_led = AMARELO;
                break;
            case 0b00100000: // Ciano: G = B = 1, R = 0
                GPIOD->ODR &= ~GPIO_ODR_OD12_Msk; // Apaga vermelho
                GPIOD->ODR |= GPIO_ODR_OD14_Msk; // Acende verde
                GPIOD->ODR |= GPIO_ODR_OD15_Msk; // Acende azul
                LEDInterativo.cor_led = CIANO;
                break;
            case 0b01000000: // Magenta: R = B = 1, G = 0
                GPIOD->ODR |= GPIO_ODR_OD12_Msk; // Acende vermelho
                GPIOD->ODR &= ~GPIO_ODR_OD14_Msk; // Apaga verde
                GPIOD->ODR |= GPIO_ODR_OD15_Msk; // Acende azul
...
//Configuração do contador (falta configurar direitinho)
